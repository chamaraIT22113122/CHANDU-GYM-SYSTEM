...
export default function Sidebar() {
  ...
  const [role, setRole] = useState<string | null>("ADMIN"); // Hardcoded for demo until fully migrated
  ...
  return (
    <div className="h-screen w-64 bg-[#1a1a1a] border-r border-white/5 flex flex-col justify-between hidden md:flex sticky top-0">
      <div>
        <div className="h-20 flex items-center px-6 border-b border-white/5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full overflow-hidden border border-[#ccff00]/50 bg-white/5 p-1 flex-shrink-0">
              <img 
                src="/logo.jpg" 
                alt="Chandu GYM" 
            ...
            <span className="text-white font-bold tracking-wider text-lg">CHANDU<span className="text-[#ccff00]">GYM</span></span>
          </div>
        </div>
              ...
              <Link key={link.name} to={link.href}>
                <motion.div 
                  ...
                  className={`flex items-center gap-3 px-3 py-3 rounded-xl transition-all ${
                    isActive 
                      ? "bg-[#ccff00]/10 text-[#ccff00] font-medium border border-[#ccff00]/20" 
      ...
      <div className="p-4 border-t border-white/5">
        <button onClick={handleLogout} className="flex items-center gap-3 px-3 py-3 w-full rounded-xl text-gray-400 hover:text-red-500 hover:bg-red-500/10 transition-all text-left">
